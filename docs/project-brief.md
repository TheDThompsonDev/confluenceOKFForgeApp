...
**Series plan:** V1 the app + the bet (above). V2 the ingest agent: writing the wiki-constitution page, batch ingest via Remote MCP, "ingest everything while I sleep." V3 the platform play: content properties for OKF metadata, Teamwork Graph API queries, co-marketing window with the EAP team if available.
